...
bool PGSymmetry::isRegularPolygon( vector < uint > idx)
{
    uint size0=idx.size();
...
bool PGSymmetry::CheckS2n(vector3 vec, uint order)
{
...
bool PGSymmetry::CheckCn(vector3 vec, uint  order)
{
    if (vec.length() < 0.1) return false;
...
uint PGSymmetry::detectAxisOrder(vector3 vec)
{
    uint order0=1;
    uint maxOrder=1;
...
bool PGSymmetry::CombinationCn(vector < uint > & arr, uint data[],
...
void PGSymmetry::SearchCn(uint order0, vector < vector3 > & container)
{
    double eps = tol;
...
void PGSymmetry::PGSymmetry::Orientation_T()
{
...
//---------------------------------------
void PGSymmetry::buildOperation_Cn(uint n)
{
    matrix3x3 matrix;
    uint i;
...
//---------------------------------------
void PGSymmetry::buildOperation_S2n(uint n)
{
    matrix3x3 matrix;
    int i;
...
//---------------------------------------
void PGSymmetry::buildOperation_Cnv(uint n)
{
    matrix3x3 matrix;
...
//---------------------------------------
void PGSymmetry::buildOperation_Cnh(uint n)
{
    matrix3x3 matrix;
    angle=360.0/double(n);
    double ang=0.0;
...
//---------------------------------------
void PGSymmetry::buildOperation_Dn(uint n)
{
    matrix3x3 matrix;
    uint i;
    angle=360.0/double(n);
...
//--------------------------------------------------
void PGSymmetry::buildOperation_Dnd(uint n)
{
    matrix3x3 matrix;
    int i;
    angle=360.0/double(n);
...
//--------------------------------------------
void PGSymmetry::buildOperation_Dnh(uint n)
{
...
void PGSymmetry::patchMolecule(uint & nAtomsPatched)
{
    ParseSymmetrySybmol(PGSymbol);
    GetData();
   ...
   for (uint i=0;i < SubGroups.size();i++)
   {
       ...
       for(uint j=1; j < operationCount ; j++)
       {
            ...
            cout << vec1<<endl;
            bool existed=false;
            for (uint k=0; k < SubGroups[i].size (); k++) //vector < vector <uint> >   SubGroups;
            {
...
*/
    ...
    for (uint i=0; i < SubGroups.size(); i++)
    {
        ...
        vector <uint> :: iterator iter =  unique ( SubGroups[i].begin(), SubGroups[i].end());
