...
void EHMO::clearMolBasisSets()
{
    ...
    MolBasisSets.clear();
    QVector<SlateBasisSets*>().swap(MolBasisSets);
}
...
void EHMO::buildBasis()
{
    ...
    QStringList S1={"S"};
    QStringList P3={"Px","Pz","Py"}; //1,0,-1
    QStringList D5={"Dxy","Dyz","Dz2","Dxz","Dx2-y2"};  //2,1,0,-1,-2
    QStringList F7={"Fy(3x2-y2)","Fxyz","Fyz2","Fz3","Fxz2","Fz(x2-y2)","Fx(x2-3y2)"}; //3 --> -3
    ...
    for(unsigned i=0;i<mol->NumAtoms();i++)
    {
        ...
        for (unsigned j=0;j<ElectronicStructure.size();j++)
        {
            ...
            electronStruct=line.split(" ",QString::SkipEmptyParts);
        ...
        for (unsigned int j=0;j<EHMO_Data.size();j++)
        {
            ...
            if(l==0)
            {
                ...
                //eV ---> Hartree
                bas->IP=single[6].toDouble()/27.2114;
                ...
                for (int m=l; m>=-l; m--)
                {
                    ...
                    //eV ---> Hartree
                    bas->IP=single[6].toDouble()/27.21165;
...
void EHMO::runEHMO()
{
    ...
    for (int i=0;i<numBS; i++)
    {
        ...
        {
            OverlapMatrix(i,j) =SlaterOverlap(MolBasisSets[i],MolBasisSets[j]);
    ...
    for (int i=0;i<numBS; i++)
    {
        ...
        for (int j=i+1;j<numBS; j++)
        {
            FockMatrix(i,j) = 0.5*1.75 * OverlapMatrix(i,j) * (MolBasisSets[i]->IP + MolBasisSets[j]->IP);
    ...
    //FC=SCE --->Ft*Ct=Ct*E
    //(A' * F * A)* (invA * C) = (invA * C)*E
    //Ft=A' * F * A    Ct=invA*C   C=A*Ct
    // where : A'SA=1

    //construct transformation matrix A, where A'SA=1
    DDMatrix eigVal, eigVec;
    OverlapMatrix.eigen_space (eigVal, eigVec, true);
/*
    std::cout << "The eigenvalue of OverlapMatrix" << std::endl;
    eigVal.dump (std::cout) << std::endl;

    std::cout << "The eigenvector of OverlapMatrix" << std::endl;
    eigVec.dump (std::cout) << std::endl;
*/

    DDMatrix B(numBS,numBS,0.0);;
    for(int i=0;i<numBS;i++)
        B(i,i) = 1.0/sqrt(eigVal(0,i));

    DDMatrix A=eigVec*B;


    //std::cout << "The sqrt DMatrix:" << std::endl;
    //B.dump (std::cout) << std::endl;

    //std::cout << "The transform matrix of DMatrix:" << std::endl;
    ...
    //std::cout << "The Ft matrix of DMatrix" << std::endl;
    //Ft.dump (std::cout) << std::endl;

    //std::cout << "The Energy matrix:" << std::endl;
    //eigVal.dump (std::cout) << std::endl;


    DDMatrix C=A*eigVec,Ct=C;
    //std::cout << "The coefficient matrix:" << std::endl;
    //C.dump (std::cout) << std::endl;

/*
    //!direct Method II
    //FC=SCE <---> (invS*F)*C=C*E
    DDMatrix invS=OverlapMatrix.invert();
    ...
    for (int i = 0; i<numBS;i++)
    {
        for (int j = 0; j<numBS;j++)
        {
            ...
            for (int k = 0; k<numBS; k++)
                Ft(i,j) += invS(i,k) * FockMatrix(k,j);
        }
    }
    Ft.eigen_space (eigVal, C, true);
...
*/
    ...
    for (int k = 0; k < numBS; k++)
    {
        ...
        for (int p = 0; p < numBS; p++) {
            for (int q = 0; q < numBS; q++) {
                ...
                normalizationFactor += C(p,k)* OverlapMatrix(p,q) * C(q,k) ;
            }
        }
        ...
        //here: C(p,k) = C(k,p)
        for (int p = 0; p < numBS; p++) {
            C(p,k) = C(p,k)/ normalizationFactor;
        }
    }
    ...
    for(int i=numBS-1; i>-1; i--)
    {
        huckelEigValues.push_back(eigVal(0,i)*27.2114);
        vector < double > v;
        for(int j=0;j<numBS;j++)
            v.push_back(C(j,i));
