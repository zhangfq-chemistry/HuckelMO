...
int rootfind_cx(mpz_t *p, int len, mpc_t start,mpc_t root,int log10_thresh, int PRECISION){
    ...
    mpz_t *pp; pp=(mpz_t*)malloc((len)*sizeof(mpz_t)); //valgrind sees an issue here
    mpz_t *ppp; ppp=(mpz_t*)malloc((len)*sizeof(mpz_t)); //or here
    ...
    while(mpfr_cmp(diff,thresh)>=0&&(c<max_iterates)){
        ...
        mpc_abs(diff,quot,(mpfr_rnd_t)MPC_RNDNN);
...
int polydivide_r(mpq_t *p,mpq_t *d,mpq_t *r,int len){
    ...
    mpq_t *outq=(mpq_t*)malloc(len*sizeof(mpq_t)); 
...
void gcd(mpz_t *poly1, mpz_t *poly2, mpz_t *gcd, int poly_len){
    ...
    mpq_t lc; mpq_init(lc);
    mpq_t *a=(mpq_t*)malloc(poly_len*sizeof(mpq_t));//temporary polynomials
    mpq_t *b=(mpq_t*)malloc(poly_len*sizeof(mpq_t));
    mpq_t *r=(mpq_t*)malloc(poly_len*sizeof(mpq_t));
...
int find_factor_cx(mpz_t *poly, mpz_t *d, mpz_t *q, int poly_len,int PRECISION,int verbosity, double d_delta, int stop_deg){
    ...
    mpc_real(output_r,output,(mpfr_rnd_t)MPC_RNDNN);
    mpc_imag(output_i,output,(mpfr_rnd_t)MPC_RNDNN);
    ...
    basis=(mpz_t*)malloc((input_degree+1)*(input_degree+3)*sizeof(mpz_t));
...
int factorize(mpz_t *poly,int poly_len,int PRECISION,mpz_t *factors,int verbosity, double delta, int stop_deg){
    ...
    d=(mpz_t*)malloc(poly_len*sizeof(mpz_t));//divisor for intermediate step
    q=(mpz_t*)malloc(poly_len*sizeof(mpz_t));//quotient for intermediate step
    for(i=0;i<poly_len;i++){
        mpz_init(d[i]);
        mpz_init(q[i]);
        mpz_set_ui(d[i],0);
...
int factorize_full(mpz_t *poly,int poly_len,int PRECISION,mpz_t *factors, int *multiplicities,int verbosity, double delta, int stop_deg){
    ...
    mpz_t *p=(mpz_t*)malloc(poly_len*sizeof(mpz_t));
    mpz_t *pp=(mpz_t*)malloc(poly_len*sizeof(mpz_t));
    mpz_t *gcd_p=(mpz_t*)malloc(poly_len*sizeof(mpz_t));
    mpz_t *stripped=(mpz_t*)malloc(poly_len*sizeof(mpz_t));
