...
void create_basis_cx(mpz_t *mat,mpc_t alpha,int deg,int sig_digits, int PRECISION){
    ...
    for(i=0;i<deg+1;i++){
        ...
        mpc_real(alpha_r,alpha_power,(mpfr_rnd_t)MPC_RNDNN); //get real and imag parts
        mpc_imag(alpha_i,alpha_power,(mpfr_rnd_t)MPC_RNDNN);
...
int sig_mpc(mpc_t alpha,int deg,int PRECISION){
    ...
    mpc_abs(norm,alpha,(mpfr_rnd_t)MPC_RNDNN);
...
void gram_schmidt(int start,int dim, int nvec, mpz_t *basis, mpf_t *obasis, int PRECISION){
    int i,j,k,s;	
    mpf_t *dummy;
    dummy=(mpf_t*)malloc(dim*sizeof(mpf_t));
...
void LLL(int dim, int nvec, mpz_t *basis, mpf_t delta, int PRECISION){
    ...
    mpf_t *obasis;
    obasis=(mpf_t*)malloc(dim*nvec*sizeof(mpf_t));
