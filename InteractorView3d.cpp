...
uint SelectAreaInteractorStyle::numSelectedObjects()
{
    return (view3d->numSelectedAtoms()+view3d->numSelectedBonds()+view3d->numSelectedRings());
}


uint SelectAreaInteractorStyle::numSelectedAtoms()
{
    return view3d->numSelectedAtoms();
}

uint SelectAreaInteractorStyle::numSelectedBonds()
{
    return view3d->numSelectedBonds();
}

uint SelectAreaInteractorStyle::numSelectedRings()
{
...
void  SelectAreaInteractorStyle::rotateSelectedAtoms()
{
    ...
    double boxCenter[3];
    vector3 obj_centerV=view3d->getOriginSelectedAtoms();
    boxCenter[0]=obj_centerV[0];
    boxCenter[1]=obj_centerV[1];
    boxCenter[2]=obj_centerV[2];

    double boundRadius = view3d->getLengthSelectedAtoms()*0.5;
    if(boundRadius<0.3) boundRadius=1.0;
...
*/
    ...
    this->ComputeWorldToDisplay(boxCenter[0], boxCenter[1], boxCenter[2], disp_obj_center);
...
void SelectAreaInteractorStyle::OnKeyPress()
{
    ...
    if(key == "a")
    {
        ...
        {
            view3d->setSelectedAtomSymbol("Al");
            return;
        }
    ...
    if(key == "k")
    {
        view3d->setKekuleStyle(!view3d->isKekuleStyle());
        view3d->refresh();
        return;
    }
    ...
    if(key == "n")
    {
        if(view3d->numSelectedAtoms()>0)
        {
            view3d->setSelectedAtomSymbol("N");
            return;
        }
...
void SelectAreaInteractorStyle::linkMolecule()
{
    ...
    if(pickedActor->Id()>100000) return;
    if(pickedActor->AtomId()>100000) return;
...
void SelectAreaInteractorStyle::OnLeftButtonDown()
{
    ...
    if(moveDistance > 5)   NumberOfClicks = 1;
...
/*
        ...
        {
            uint j=view3d->selectedBondList[0][0];
            uint bo=view3d->getBondOrderbyId(j);
            cout << j<< "  "<< bo<< " sfsdfs1"<<endl;
            if(bo==6) {
               mol->getBondbyAtomIndex(view3d->selectedBondList[0][1],view3d->selectedBondList[0][2])->setIndex(
                           view3d->selectedBondList[0][2],view3d->selectedBondList[0][1]);
...
void SelectAreaInteractorStyle::OnMouseMove()
{
    ...
    if(pickedActor)  // dynamic line from this atom
    if(view3d->isPencilDrawMode())
    {
        ...
        if(!tmpLineActor || tmpLineActor==nullptr)
        {
            tmpLineActor = vtkActor::New();
            tmpLineActor ->setMOId(-1);
    ...
    if(numSelectedAtoms()>0 && (isShiftPressed || isControlPressed) )
    {
...
void SelectAreaInteractorStyle::disposeSingleSelectedActor(vtkActor * actor)
{
    ...
    if(mouseOperationMode==VTKPencilDrawing)
    {
        ...
        if(actor->AtomId()>=0 && actor->AtomId1()<0)
        {
            ...
            //cout << "actor->MOId() : "<< actor->MOId() <<endl;
           // cout << "actor->AtomId() : "<< actor->AtomId() <<endl;
            //cout << "actor->AtomId1() : "<< actor->AtomId1() <<endl;
    ...
    if(actor->AtomId()>=0 && actor->AtomId1()<0 )
    {
        ...
        else
        {
            ...
            view3d->selectedAtomActors->AddItem(actor);
            highlightActor(actor);
            //buildAtomLabel(actor);
        }

        /*
        //display  selected atoms list
        view3d->selectedAtomActors->InitTraversal();
        for (uint i=0;i< view3d->selectedAtomActors->GetNumberOfItems();++i)
        {
            cout<< view3d->selectedAtomActors->GetNextActor()->AtomId()<<endl;
        }
*/
...
void SelectAreaInteractorStyle::OnLeftButtonUp()
{
    ...
    if(mouseOperationMode==VTKPencilDrawing)
    {
        ...
        if ( picker != nullptr )
        {
            ...
            if(pickedActor)
            {
               ...
               // cout << "pickedActor->AtomId: "<< pickedActor->AtomId()<<endl;
               // cout << "pickedActor->BondId: "<< pickedActor->BondId()<<endl;
               // cout << "pickedActor->getRingId: "<< pickedActor->getRingId()<<endl;
    ...
    //store bonds
    std::vector <uint> singleBonds,otherBonds;

    vtkPlanes* frustum = areaPicker->GetFrustum();
    uint numSelected=areaPicker->AreaPick(min[0], min[1], max[0], max[1], GetDefaultRenderer());
    ...
    uint numSelAtoms=0,numSelBonds=0,numSelRings=0;
    ...
    for (uint i=0;i<areaPicker->NumSelectedProps() ;i++)
    {
    ...
    for (uint i=0;i<_atomActors->GetNumberOfItems(); i++)
    {
    ...
    for (uint i=0;i<numSelBonds; i++)
    {
        ...
        for (uint j=0;j<areaPicker->NumSelectedProps () ;j++)
        {
...
//help function
void SelectAreaInteractorStyle::collectLeftHalfBonds(vector <uint > v)
{
...
bool SelectAreaInteractorStyle::isAtomPicked(uint id)
{
...
vector3 SelectAreaInteractorStyle::getHitModelPositionBasedonLinkeredActor(uint x, uint y, vtkActor * ref)
{
...
vector3 SelectAreaInteractorStyle:: getHitModelPosition(uint x, uint y)
{
    double worldpos[4];
...
vector3 SelectAreaInteractorStyle:: getHitModelPosition(uint p[2])
{
...
void SelectAreaInteractorStyle::contextMenu()
{
    ...
    uint numSelAtoms=view3d->numSelectedAtoms();
    uint numSelBonds=view3d->numSelectedBonds();
    uint numSelRings=view3d->numSelectedRings();
    uint numSelObj=numSelAtoms+numSelBonds+numSelRings;
 ...
 //   cout << " selected bonds "<< numSelBonds<<endl;
  //  cout << " selected rings "<< numSelRings<<endl<<endl;;
    ...
    QAction actionDelete ("delete" ,view3d);
   ...
   // if(numSelAtoms>0)
    ...
    {
        QAction actionPaste ("paste" ,view3d);
    ...
    if(view3d->numSelectedObjects()<1){
...
{
     view3d->setHydrogenVisible(view3d->isHydrogenHide());
     view3d->refresh();
}
