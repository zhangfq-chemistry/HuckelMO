...
public:
    ...
    Private(ColorWheel *widget)
        ...
        color_from([](qreal h, qreal s, qreal v, qreal a = 1.0) { return QColor::fromHsvF(h, s, v, a); }), rainbow_from_hue(&detail::rainbow_hsv)
    {
        qreal backgroundValue = qGray(widget->palette().color(QPalette::Window).rgb()) / 255.0;
...
void ColorWheel::setDisplayFlags(DisplayFlags flags)
{
    ...
    if ( (flags & COLOR_FLAGS) != (p->display_flags & COLOR_FLAGS) )
    {
        ...
        else
        {
            ...
            p->color_from = [](qreal h, qreal s, qreal v, qreal a = 1.0) { return QColor::fromHsvF(h, s, v, a); };
            p->rainbow_from_hue = &detail::rainbow_hsv;
        }
        p->render_ring();
    }
